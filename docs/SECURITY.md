...
| Payment without delivery, or delivery without payment | `(try! (stx-transfer? cost tx-sender creator))` runs before any balance/supply mutation. If it fails, the whole call aborts and Clarity rolls back every state change made during it — there is no code path where tokens move without the matching STX payment succeeding, or vice versa. See "Atomicity" below. |
...
| Price manipulation | `price` is set once, at `create-token` time, by the creator, and has no setter afterward — a buyer's cost (`amount * price`) can't be changed after they've seen it, and the frontend computes the identical multiplication the contract does before showing "Total". |
...
| Unauthorized withdrawal of STX | There is no admin, owner, or protocol account with any special privilege in this contract at all — no `set-*-admin`, no pause switch, no fee. STX only ever moves buyer → creator, inside `purchase`, for the exact `amount * price` computed from on-chain state. |
...
amount, self-purchase, insufficient supply, or a failed `stx-transfer?`
(insufficient buyer STX) — leaves both the `tokens` and `balances` maps
exactly as they were. The 21 tests in
`contracts/tests/token-market.test.ts` include explicit before/after STX
balance assertions (via `simnet.getAssetsMap()`) proving the payment
actually moves on a successful call, not just that the function returns
`ok`.
...
- **STX balance pre-checks are advisory, not authoritative.** The purchase
  form checks the buyer's STX balance against the purchase cost before
  prompting the wallet, purely to give a clearer error message than a
  failed broadcast would. The contract's own `stx-transfer?` call is the
  real enforcement point regardless of what the frontend checked.
...
every validated input (empty name/symbol/category, zero supply/price, zero
purchase amount, over-supply purchase, self-purchase).
